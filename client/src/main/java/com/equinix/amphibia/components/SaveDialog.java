...
public class SaveDialog extends javax.swing.JPanel {
    ...
    public boolean validateAndSave(String title, boolean isRestore) {
        ...
        list.forEach((path) -> {
            try {
                File target = IO.newFile(path.toString());
                File source = IO.getBackupFile(target);
                File dir = IO.newFile(path.toString()).getParentFile();
                ...
                target = IO.newFile(dir, "data/profile.json");
        ...
        if (isSame && !isRestore) { //try to delete all backup files
            list.forEach((path) -> {
               File profileFile = IO.newFile(path.toString());
               File bak = IO.getBackupFile(profileFile);
               if (bak.exists()) {
                    bak.deleteOnExit();
                    bak.delete();
               }
               bak = IO.getBackupFile(IO.newFile(profileFile.getParentFile(), "data/profile.bak"));
               if (bak.exists()) {
                    bak.deleteOnExit();
                    bak.delete();
               }
